...
def set_feature(view,f):
  ...
  return ((view.split(' ')[0],str(f)+'.txt'),x)
...
def get_target_info():
	...
	collection = tfile.map(lambda view: (view.split(' ')[0],float(view.split(' ')[2]))).reduceByKey(lambda a,b: a+b).collect()
	target = {}
	for data in collection:
